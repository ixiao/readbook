...
- (void)openBookWithChapterIndex:(NSInteger)chapterIndex
{
    if (!_readerBook) {
        ...
        _readerBook.bookName = [self.model objectForKey:@"name"];
        _readerBook.totalChapter = 0;
        _readerBook.urlPath = [self.model objectForKey:@"url"];
    }
...
- (void)showReaderSettingBar
{
    ...
    topBar.title.text = [self.model objectForKey:@"name"];
